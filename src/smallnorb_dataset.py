""" SmallNORB dataset wrapper
Based on `torchvision.datasets.MNIST` and `https://cs.nyu.edu/~ylclab/data/norb-v1.0-small/`
"""
import errno
import os
import os.path
...
class smallNORB(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None:
            img0 = self.transform(Image.fromarray(img.numpy()[0], mode='L'))
            img1 = self.transform(Image.fromarray(img.numpy()[1], mode='L'))
...
def read_label_file(path):
    with open(path, 'rb') as f:
        f.read(20)
        data = f.read()
        parsed = np.frombuffer(data, dtype=np.int32)
        return torch.from_numpy(parsed).long()
...
def read_image_file(path):
    with open(path, 'rb') as f:
        ...
        data = f.read()
        parsed = np.frombuffer(data, dtype=np.uint8)
        return torch.from_numpy(parsed).view(*dimensions)
